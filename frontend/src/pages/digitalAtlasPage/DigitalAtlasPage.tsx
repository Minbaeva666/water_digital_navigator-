...
import {FilterOutlined, ReloadOutlined, SearchOutlined, SortAscendingOutlined} from "@ant-design/icons";
...
import TaxonomyFilterNav, {PickedNode} from "../../components/taxonomyFilterNav/TaxonomyFilterNav.tsx";
import "./DigitalAtlasPage.less";
import {mapBackendToSolutionsWithCoords, SolutionWithCoords} from "../../utils/map.helper.tsx";
...
const DigitalAtlasPage = () => {
    const [digitalSolutions, setDigitalSolutions] = useState<DigitalSolutionDto[]>([]);
    ...
    }), [current, pageSize, picked?.id, picked?.path, query, sortBy, dateFrom, dateTo]);
        ...
        const load = async () => {
            ...
            try {
                ...
                if (Array.isArray(backendCoords)) {
                    const mapped = mapBackendToSolutionsWithCoords(backendCoords)
            ...
            <div style={{marginBottom: 16}}>
                <Space wrap size={[8, 8]}>
                    ...
                    <Button
                        ...
                        disabled={!picked && !query && sortBy === "newest" && !dateFrom && !dateTo}
                    >
                        Zurücksetzen
                    </Button>
                </Space>
                ...
                <Col xs={24} md={17} lg={18} xl={18} xxl={19}>
                    <div className="solutions-area">
                        {digitalSolutions.length === 0 ? (
                            <Empty
                                description={picked ? "Keine Treffer für diesen Filter" : "Keine digitalen Lösungen gefunden"}
                            />
                        ) : (
                            <div className="cards-grid">
                                {digitalSolutions.map((sol) => (
                                    <div key={sol.id} className="card-cell">
                                        <DigitalSolutionCard digitalSolution={sol} taxonomyIndex={taxonomyIndex}
