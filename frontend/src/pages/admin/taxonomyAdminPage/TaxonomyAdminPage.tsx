...
import {Button, Col, Input, message, Modal, Row, Spin, Tooltip, Typography} from "antd";
import "./TaxonomyAdminPage.less";
import {PlusOutlined, SaveOutlined, UndoOutlined, ShrinkOutlined, ExclamationCircleOutlined} from "@ant-design/icons";
...
import {
    ...
    flattenNodesForSearch, getParentKeyFromTree,
    slugify,
    sortByName,
    updateNodeRecursive
...
const tempId = () => (crypto?.randomUUID ? crypto.randomUUID() : `tmp-${Date.now()}-${Math.random().toString(36).slice(2)}`);


const TaxonomyAdminPage = () => {
    const MAX_DEPTH = 3;
    const [taxonomyNodes, setTaxonomyNodes] = useState<TaxonomyNodeDto[]>([]);
    const [initialTaxonomyNodes, setInitialTaxonomyNodes] = useState<TaxonomyNodeDto[]>([]);
    ...
    const addLocalChildNode = (parent: LocalTaxonomyNode, values: Partial<TaxonomyNodeDto>) => {
        ...
        const insertChildRecursive = (nodes: LocalTaxonomyNode[]): LocalTaxonomyNode[] => {
    ...
    const handleResetTree = () => {
        Modal.confirm({
            ...
            content: "Möchten Sie wirklich alle Änderungen verwerfen? Der Baum wird auf den zuletzt gespeicherten Stand zurückgesetzt.",
            okText: "Ja, verwerfen",
            okType: "danger",
            cancelText: "Abbrechen",
            onOk: () => {
    ...
    const flattenNodes = useCallback((nodes: LocalTaxonomyNode[]): LocalTaxonomyNode[] => {
    ...
    const isNameTakenGlobal = useCallback(
        (name: string, selfId?: string) => {
            ...
            return all.some(n => n.id !== selfId && slugify(n.nameDe || "") === target);
        },
    ...
    const confirmSave = () => {
        Modal.confirm({
            ...
            content: "Wollen Sie wirklich den Baum mit den überarbeiteten Kriterien speichern? Diese Aktion kann nicht rückgängig gemacht werden.",
            okText: "Ja, speichern",
            cancelText: "Abbrechen",
            onOk: saveCriteria,
        });
                        ...
                        <Row gutter={8} wrap={false}>
                            ...
                            <Col>
                                <Tooltip title="Änderungen Rückgängig machen">
                                    <Button danger icon={<UndoOutlined/>} onClick={handleResetTree}
                                            disabled={!hasChanges}/>
                                </Tooltip>
                            </Col>
                        </Row>
            ...
            <div className="create-edit-body-taxonomy">
                {loading ? (
                    ...
                    <div style={{
                        ...
                        Noch keine Kriterien angelegt, fügen Sie das erste Root-Kriterium hinzu.
                    </div>
                ) : (
                        <TaxonomyTree
                            maxDepth={MAX_DEPTH}
