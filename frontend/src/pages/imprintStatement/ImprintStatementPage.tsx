...
// import "./ImprintStatementPage.less";
...
const ImprintStatementPage: React.FC = () => {
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [imprint, setImprint] = useState<ImprintStatementDto | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                setError(null);
                const data = await imprintService.fetchImprintStatement();
                if (!alive) return;
                setImprint(data ?? null);
            } catch (e: any) {
                if (!alive) return;
    ...
    const updatedAt =
        imprint?.updatedAt ? new Date(imprint.updatedAt).toLocaleDateString() : undefined;

    return (
        <div className="imprint-statement-page">
            <Row justify="center">
                <Col xs={24} md={22} lg={16} xl={14}>
                    ...
                    {!loading && !error && (!imprint || !imprint.content?.trim()) && (
                        <Alert
                            type="info"
                            showIcon
                            message="Kein Impressum vorhanden"
                            description="Aktuell sind noch keine Impressumsangaben hinterlegt."
                        />
                    )}

                    {!loading && !error && imprint?.content?.trim() && (
...
export default ImprintStatementPage;
