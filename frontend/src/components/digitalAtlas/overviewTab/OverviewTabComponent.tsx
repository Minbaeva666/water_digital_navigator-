...
import { PhoneOutlined, MailOutlined, BankOutlined, GlobalOutlined, EnvironmentOutlined } from "@ant-design/icons";
...
import {getSourceCountryName, getSourceRegionName} from "../../../utils/formDataHelper.ts";
...
export function OverviewTabComponent({ digitalSolution, salutationsMap, taxonomyIndex }: DigitalSolutionCardProps) {
    ...
    const getPhone = (u: any) =>
        u?.phonenumber ?? u?.phone ?? u?.phoneNumber ?? u?.mobile ?? u?.tel ?? null;

    const u = digitalSolution.presentedByUser;
    const salutationType = u?.salutationType;
    const salutationLabel =
        salutationType === SalutationType.MR || salutationType === SalutationType.MS
    ...
    const org = u?.organization ?? (digitalSolution as any).organization ?? null;
    ...
    const getOrgAddressOneLine = (o?: any | null) => {
        if (!o) return "";
        const streetWithNo = [o.street ?? o.addressStreet ?? o.streetName, o.houseNumber ?? o.streetNumber]
            .filter(Boolean)
            .join(" ")
            .trim();
        ...
        const countryLabel = rawCountry.nameDe;

        const parts = [streetWithNo || o.street, zip, city, countryLabel].filter(Boolean);
        return parts.join(", ");
    };
    ...
    const getMaturityLabel = (value?: string) => (value ? t(`maturityDegrees.${value}`, value) : "");
    const getOfferingCategoryLabel = (value?: string) => (value ? t(`offeringCategoryTypes.${value}`, value) : "");
    ...
    const getNodeName = (n: any) =>
        isDe ? n?.nameDe ?? n?.nameEn ?? n?.slug ?? "" : n?.nameEn ?? n?.nameDe ?? n?.slug ?? "";

    // Helper um aus Index den Parent zu holen – tolerant gegenüber verschiedenen Shapes
    const getById = (id?: string | null) => (id && taxonomyIndex ? taxonomyIndex[id] : undefined);
    ...
    // IDs → echte Nodes (aus taxonomyIndex) → deduplizieren
    ...
    // Gruppierung: rootId → {rootName, rootColor, items}
    const groupsMap: Record<string, Group> = nodes.reduce((acc: Record<string, Group>, node: any) => {
        const root = getRoot(node);
        const rootId = root?.id ?? "__ohne_root__";
        const rootName = root ? getNodeName(root) : t("common.uncategorized", "Allgemein");
        const rootColor = root?.color ?? undefined;

        if (!acc[rootId]) acc[rootId] = { rootId, rootName, rootColor, items: [] };
...
// Orga vorhanden? (je nachdem, was du hast: org-Objekt, organizationId oder beides)
    ...
    // Logo nur zeigen, wenn es eine Orga gibt UND KEIN presenterName vorhanden ist
        ...
        <div>
            <Row gutter={[55, 32]} style={{ paddingTop: 80 }}>
                ...
                <Col xs={24} md={12}>
                    ...
                    {showBlock && (
                        <>
                            {!!digitalSolution?.name?.trim() && (
                                <Title level={4}>{digitalSolution.name} wird präsentiert von</Title>
                            )}
                            <Space align="center" size={12}>
                                ...
                                <Text>
                                    {hasPresenter ? presenterName : (org?.name ?? "Organisation")}
                                </Text>
                            </Space>
                        </>
                    )}
                    ...
                    {digitalSolution.publishedBy && (
                        <>
                            <Title level={4} style={{ marginTop: 24 }}>
                                {t("solution.publishedBy.title", "Digitale Lösung veröffentlicht durch")}
                            </Title>
                            <Text>{getPublishedByLabel(digitalSolution.publishedBy)}</Text>

                            {showSource && (digitalSolution.publishedSource?.trim()?.length ?? 0) > 0 && (
                                <>
                                    <br />
                                    <Text type="secondary">
                                        {t("solution.publishedBy.sourceLabel", "Quelle")}: {digitalSolution.publishedSource}
                                    </Text>
                                </>
                            )}
                        </>
                    ...
                    <Text>{getOfferingCategoryLabel(digitalSolution.offeringCategory)}</Text>

                    <Title level={4} style={{ marginTop: 24 }}>Reifegrad der Digitalen Lösung</Title>
                    <Text>{getMaturityLabel(digitalSolution.maturityDegree)}</Text>

                    {(() => {
                        const ready = getReadyForOperationDate(digitalSolution.readyForOperation);
                        return ready ? (
                            <>
                                <Title level={4} style={{ marginTop: 24 }}>Betriebsbereit ab</Title>
                                <Text>{ready}</Text>
                            </>
                        ) : null;
                    })()}

                    <Title level={4} style={{ marginTop: 24 }}>Land</Title>
                    {countryName && <Text>{countryName}</Text>}

                    {regionName && (
                        <>
                            <Title level={4} style={{ marginTop: 24 }}>Bundesland / Region</Title>
                    ...
                    {u && (
                        <>
                            <Title level={4} style={{ marginTop: 24 }}>Ansprechpartner</Title>
                                ...
                                <div>
                                    ...
                                    {getPhone(u) && (
                                        <Text style={{ display: "block" }}>
                                            ...
                                            <a href={`tel:${String(getPhone(u)).replace(/\s+/g, "")}`}>{getPhone(u)}</a>
                                        </Text>
                                    )}
                    ...
                    {/* ------- NEU: Projektpartner (vor Ansprechpartner) ------- */}
                    {partners.length > 0 && (
                        <>
                            <Title level={4} style={{ marginTop: 24 }}>Projektpartner</Title>
                            <Space direction="vertical" size={14} style={{ display: "flex" }}>
                                {partners
                                    .slice()
                                    .sort((a, b) => (a?.name || "").localeCompare(b?.name || "", i18n?.language || "de"))
                                    .map((p) => {
                                        const pUrl = ensureHttp(p?.website);
                                        const pAddress = getOrgAddressOneLine(p);
                                        return (
                                            <Space key={p?.id || p?.name} align="start" size={12}>
                                                <div>
                                                    ...
                                                    {pUrl && (
                                                        <Text style={{ display: "block" }}>
                                                            <GlobalOutlined style={{ marginRight: 8 }} />
                                                            <ExternalLink href={pUrl}>{pUrl}</ExternalLink>
                                                        </Text>
                                                    )}
                                                </div>
                                            </Space>
                            ...
                            <Row>
                                {groupedAll.map((group) => (
                                    <Col xs={24} md={24} key={group.rootId}>
                                        <div className="category-group" style={{marginTop: 40}}>
