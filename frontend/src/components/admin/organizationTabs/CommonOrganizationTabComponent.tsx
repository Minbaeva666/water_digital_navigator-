...
export const CommonOrganizationTabComponent: React.FC<CommonOrganizationTabProps> = ({form}) => {
    ...
    const isMunicipality = orgType === "MUNICIPALITY";
            ...
            <Row gutter={64}>
                ...
                <Col xs={24} md={12} xl={10}>
                    ...
                    <Form.Item
                        ...
                        label={
                            <Space size="small" wrap={false} align="center">
                                ...
                                >
                                    <Button type="link" icon={<InfoCircleOutlined/>} className="pa-link-button" style={{padding: 0, height: "auto"}}>
                                        Hilfe
                                    </Button>
                                </Popover>
                            </Space>
                    ...
                    >
                        ...
                        {/*<Select placeholder="Organisationsart auswählen" options={organizationStates}/>*/}
                    ...
                    <div key={`left-${orgState ?? "none"}`}>
                        {(isLite || isFull) && (
                            <>
                                <Form.Item name="name" label="Name" rules={[{required: true, message: "Name ist erforderlich"}]} preserve={false}>
                                    <Input placeholder="Name der Organisation"/>
                                </Form.Item>

                                {isFull && (
                                    <>
                                        <Form.Item name="email" label="Kontakt-E-Mail" rules={[{required: true, type: "email", message: "Gültige E-Mail ist erforderlich"}]} preserve={false}>
                                            <Input placeholder="kontakt@domain.de"/>
                                        </Form.Item>

                                        <Form.Item name="website" label="Webseite" rules={[{required: true, type: "url", message: "Gültige URL ist erforderlich"}]} preserve={false}>
                                            <Input placeholder="https://www.beispiel.de"/>
                                        </Form.Item>
                                    </>
                                )}

                                <Form.Item name="organizationType" label="Organisationstyp" rules={[{required: true, message: "Typ ist erforderlich"}]} preserve={false}>
                                    <Select placeholder="Typ auswählen" loading={loadingTypes} disabled={loadingTypes}>
                ...
                <Col xs={24} md={12} xl={10}>
                    <div key={`right-${orgState ?? "none"}`}>
                        {(isLite || isFull) && (
                            <>
                                {isFull && (
                                    <Form.Item name="street" label="Straße" rules={[{required: true, message: "Straße ist erforderlich"}]} preserve={false}>
                                ...
                                <Form.Item name="countryCode" label="Land" rules={[{required: true, message: "Land ist erforderlich"}]} preserve={false}>
                                    <Select
                                ...
                                <Form.Item label={null}>
                                    <Space size="small" align="center" wrap={false}>
                                        ...
                                        <Popover
                                            ...
                                            trigger="click"
                                            styles={{
                                                body: { maxWidth: 320, whiteSpace: "normal", overflowWrap: "anywhere" },
                                            }}
                                        >
                                            <Button
                                                type="link"
                                ...
                                {/* Lat/Lon – nur editierbar, wenn manuell */}
                                <Form.Item
                                    name="lat"
                                    ...
                                    rules={
                                        manualCoords
                                            ? [
                                                { type: "number", min: -90, max: 90, message: "Wert muss zwischen -90 und 90 liegen" },
                                            ]
                                            : []
                                    }
                                >
                                ...
                                <Form.Item
                                    ...
                                    rules={
                                        manualCoords
                                            ? [
                                                { type: "number", min: -180, max: 180, message: "Wert muss zwischen -180 und 180 liegen" },
                                            ]
                                            : []
                                    }
                                >
