...
import jwt, { JsonWebTokenError, TokenExpiredError, SignOptions, Secret } from "jsonwebtoken";
...
export const login = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!email || !password) {
        res.status(400).json({ error: "Email and password required.", reason: "missing_credentials" });
        return;
    }
    ...
    try {
        ...
        if (!user || user.password == null) {
            res.status(401).json({ error: "Invalid login details.", reason: "invalid_credentials" });
            return;
        }
        ...
        const payload = { id: user.id, role: user.role as Role, organizationId: user.organizationId ?? null };
        ...
        const expMs = getJwtExpMs(refreshToken) ?? (Date.now() + 7 * 24 * 60 * 60 * 1000);
        const refreshHash = sha256(refreshToken);

        // Optional: nur eine Session pro User -> alte Einträge löschen
        await prisma.token.deleteMany({ where: { userId: user.id, tokenType: TokenType.REFRESH_TOKEN } });
        ...
        res.status(500).json({ error: "Server error during login.", reason: "login_failed" });
    }
};
...
export const refresh = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!raw) {
        res.status(401).json({ error: "No refresh token available.", reason: "no_refresh_token" });
        return;
    }
    ...
    try {
        ...
        if (!decoded?.id || !decoded?.role) {
            res.status(401).json({ error: "Invalid refresh token payload.", reason: "refresh_token_invalid_payload" });
            return;
        }
        ...
        const row = await prisma.token.findUnique({ where: { tokenHash: currentHash } });
        if (!row || row.tokenType !== TokenType.REFRESH_TOKEN) {
            res.status(401).json({ error: "Refresh token not recognized.", reason: "refresh_token_invalid" });
            return;
        }
        ...
        if (row.expiresAt.getTime() <= Date.now()) {
            ...
            res.status(401).json({ error: "Refresh token expired.", reason: "refresh_token_expired" });
            return;
        }
        ...
        const newAccessToken  = jwt.sign(payload, ACCESS_SECRET,  { expiresIn: ACCESS_EXPIRES_IN });
        const newRefreshToken = jwt.sign(payload, REFRESH_SECRET, { expiresIn: REFRESH_EXPIRES_IN });
        const newExpMs        = getJwtExpMs(newRefreshToken) ?? (Date.now() + 7 * 24 * 60 * 60 * 1000);
    ...
    try {
        ...
        if (raw) {
            await prisma.token.deleteMany({
                where: { tokenHash: sha256(raw), tokenType: TokenType.REFRESH_TOKEN },
            });
        }
        ...
        res.status(500).json({ error: "Server error during logout.", reason: "logout_failed" });
    }
};
